...
class _DoAdvertisePageState extends State<DoAdvertisePage> {
  ...
  List<String> participants = []; // Aquesta llista conté els noms dels participants
  ...
  Future<void> _selectDate(BuildContext context) async {
    final DateTime? picked = await showDatePicker(
      ...
      lastDate: DateTime(2100), // No hi ha límit real, però cal posar una data molt futura
  ...
  Future<void> _selectTime(BuildContext context) async {
    ...
    if (picked != null) {
      setState(() {
        selectedTime = picked;
        noTimeSelected = false; // Si l'usuari selecciona una hora, desactivem "noTimeSelected"
      });
    }
  }
  ...
  void _uploadNewAdvertise() async {
    if (_currentUserId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Error: No s\'ha pogut obtenir l\'ID de l\'usuari creador.')),
      );
      return;
    }
    ...
    if (selectedTime != null && !noTimeSelected) {
      ...
      formattedTime = null; // No enviar hora si és "Encara no ho tinc clar" o "No seleccionada"
    }
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                ChoiceChip(
                  label: Text('Esmorzar', style: TextStyle(color: selectedMeal == 'esmorzar' ? Colors.black : null)),
